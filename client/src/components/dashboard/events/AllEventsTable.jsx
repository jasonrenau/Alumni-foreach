import { Trash2, FolderEdit } from "lucide-react";
import { dateFormat } from "../../../utils/functions";
import { Link } from "react-router-dom";

const AllEventsTable = ({ allEvents, setModalId }) => {
  return (
    <div className="pt-10">
                ...
                <td className="bg-white px-4 py-5 text-sm lg:table-cell">
                  <div className="lg:flex lg:justify-center lg:gap-8">
                    <div className="max-lg:mb-8">
                      <Trash2
                        size={25}
                        onClick={() => setModalId(event.event_id)}
                        className="mx-auto cursor-pointer text-red-800 hover:scale-105"
                      />
                    </div>
                    <div className="">
                      <Link
                        to={`/dashboard/evenements/edition/${event.event_id}`}
                      >
                        <FolderEdit
                          size={25}
                          className="mx-auto cursor-pointer text-blue-800 hover:scale-105"
                        />
                      </Link>
                    </div>
                  </div>
